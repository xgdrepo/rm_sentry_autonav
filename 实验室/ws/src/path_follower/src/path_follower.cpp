...
#include <cmath>
#include <deque>
#include <std_msgs/UInt8.h>          // 新增头文件
...
private:
    ...
    void publishControlCommand() {
        ...
        // 新增：根据速度是否非零发布 spin_mode_cmd
        const double eps = 1e-6;
        if (fabs(cmd_vel.linear.x) > eps || fabs(cmd_vel.linear.y) > eps || fabs(cmd_vel.angular.z) > eps) {
            std_msgs::UInt8 spin_cmd;
            spin_cmd.data = 0;      // 运动时关闭小陀螺
            spin_pub_.publish(spin_cmd);
        }
    }
...
public:
        ...
        new_path_received_(false)
    {
        ...
        spin_pub_ = nh_.advertise<std_msgs::UInt8>("/spin_mode_cmd", 1);   // 新增发布器初始化
