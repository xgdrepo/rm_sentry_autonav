...
public:
    ...
    bool findFreeNeighbor(int cx, int cy, int search_radius, int& free_x, int& free_y) {
    ...
    void planningCallback() {
        if (!pose_received_ || !goal_received_ || !map_received_) return;
        ...
        int raw_start_x, raw_start_y, goal_x, goal_y;
        if (!worldToMap(current_pose_.pose.position.x, current_pose_.pose.position.y, raw_start_x, raw_start_y) ||
            !worldToMap(goal_pose_.pose.position.x, goal_pose_.pose.position.y, goal_x, goal_y)) {
            ROS_WARN_THROTTLE(2.0, "Cannot convert world coordinates to map!");
            return;
        }

        // 寻找实际可用的起点（如果原始起点不自由）
        int start_x = raw_start_x, start_y = raw_start_y;
        if (!isCellFree(start_x, start_y)) {
            ROS_WARN("Start cell (%d,%d) is not free, searching nearby...", start_x, start_y);
            if (!findFreeNeighbor(start_x, start_y, 5, start_x, start_y)) { // 搜索半径5格
                ROS_ERROR("No free cell near start position, cannot plan!");
                return;
            }
            ROS_INFO("Adjusted start to (%d,%d)", start_x, start_y);
        }

        // 可选：同样对终点做邻域搜索（若终点不自由）
        if (!isCellFree(goal_x, goal_y)) {
            ROS_WARN("Goal cell (%d,%d) is not free, searching nearby...", goal_x, goal_y);
            int new_goal_x, new_goal_y;
            if (findFreeNeighbor(goal_x, goal_y, 5, new_goal_x, new_goal_y)) {
                goal_x = new_goal_x;
                goal_y = new_goal_y;
                ROS_INFO("Adjusted goal to (%d,%d)", goal_x, goal_y);
            } else {
                ROS_ERROR("No free cell near goal, cannot plan!");
                return;
            }
        }
