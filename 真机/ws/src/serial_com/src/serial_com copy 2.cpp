...
std::atomic<uint8_t> spin_mode_enabled(0);  // 存储当前小陀螺状态
...
void cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg) {
    ...
    serial_package.spin_mode = current_spin_mode;  // 使用当前状态
    ...
    try {
        if(sentry_ser.isOpen()) {
            ...
            if(bytes_written == sizeof(Serial_Package)) {
                // ROS_INFO("Send data finished! Spin mode: %d", current_spin_mode);
            } else {
    ...
    // 注意：这里不再重复发布spin_mode_status，因为在spinModeCallback中已经发布了
    // 这样可以确保spin_mode_status完全跟随spin_mode_cmd
}
